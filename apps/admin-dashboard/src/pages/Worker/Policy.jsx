...
export default function Policy() {
  const { workerProfile, policyData, premiumHistoryData } = useAppContext();
  const weeklyPremium = Number(policyData?.premium || workerProfile.policy.weeklyPremium || 39);
  const maxPayout = Number(workerProfile.policy.maxPayout || 750);
  ...
  const policySpeed = speedByTier[workerProfile.tier] || "8 hours";
  const policyReference = policyData?.id ? String(policyData.id).slice(0, 12).toUpperCase() : "GS-KA-2026-9876";
  const coverageSince = policyData?.starts
    ? new Date(policyData.starts).toLocaleDateString("en-IN", { month: "short", year: "numeric" })
    : "Week 1, 2026";

  const breakdownRows = [
    ["Claim Reserve (45%)", weeklyPremium * 0.45, COLORS.green],
    ["GigCorpus Pool (30%)", weeklyPremium * 0.3, COLORS.gold],
    ["Fraud Reserve (10%)", weeklyPremium * 0.1, COLORS.purple],
    ["Platform Ops (10%)", weeklyPremium * 0.1, COLORS.blue],
    ["Reinsurance (5%)", weeklyPremium * 0.05, COLORS.muted],
  ];
      ...
      <div style={{ ...s.card, background: `linear-gradient(135deg, ${COLORS.purple}14, ${COLORS.bg})`, padding: 28 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            <h2 style={{ fontSize: 26, fontWeight: 800, marginTop: 4, color: COLORS.text }}>{workerProfile.tier} Protection Plan</h2>
            <p style={{ color: COLORS.muted, fontSize: 13, marginTop: 4 }}>Policy ID: {policyReference} · Renews every Monday 00:00</p>
          </div>
          <span style={{ ...s.tag(COLORS.green), fontSize: 12, padding: "6px 16px" }}>● ACTIVE</span>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginTop: 20 }}>
          {[["Weekly Premium", `Rs. ${weeklyPremium}`], ["Max Payout", `Rs. ${maxPayout}/week`], ["Claim Speed", policySpeed], ["Coverage Since", coverageSince]].map(([k, v]) => (
        ...
        <div style={s.card}>
          ...
          {breakdownRows.map(([k, v, c]) => (
            <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "9px 0", borderBottom: `1px solid ${COLORS.border}` }}>
              ...
              <span style={{ fontWeight: 700, color: c, fontSize: 13 }}>Rs. {Number(v).toFixed(2)}</span>
            </div>
          ))}
          <div style={{ display: "flex", justifyContent: "space-between", padding: "12px 0", marginTop: 4 }}>
            <span style={{ fontWeight: 700 }}>Total Weekly Premium</span>
            <span style={{ fontWeight: 800, color: COLORS.accent, fontSize: 16 }}>Rs. {weeklyPremium.toFixed(2)}</span>
          </div>
        </div>
        <div style={s.card}>
          <h3 style={{ fontWeight: 700, fontSize: 15, marginBottom: 14 }}>Premium History</h3>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={premiumHistoryData}>
